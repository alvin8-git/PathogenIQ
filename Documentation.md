...
4. Attributes each gene to the targeted finding(s) whose reference genome carries it, by aligning the contigs back to the screened genomes (see **Attributing genes to organisms** below)
...
**Attributing genes to organisms.** Screening contigs (rather than per-organism reference genomes) is what makes the gene calls *sample-specific* — an acquired plasmid in this isolate shows up, and a gene absent from this sample's strain does not. The cost is that an assembled contig carries only its own id (`k141_42`), not the organism it came from, so it must be attributed back to a finding. `map_contigs_to_organisms()` does this by aligning the contigs to the handful of screened genomes (minimap2) and recording, per contig, the total aligned bases against each genome.

A naïve "assign each contig to its single best genome" is wrong for **near-identical siblings**. In a co-assembly the near-identical reads of *E. coli* and *Shigella* collapse into one shared contig set; forcing a single winner buried every *Shigella* resistance/virulence gene under *E. coli* (both *Shigella* showed 0 AMR/VFDB). Instead a contig is **co-attributed** to every genome whose aligned bases are within `_COMAP_MARGIN` (0.70) of its best match. The margin is deliberately loose: a reduced sibling genome (*Shigella* has lost many segments relative to *E. coli*) covers only ~80 % of a shared contig even at high identity, so the genuine sibling band sits near 0.8 aligned-bp, not ~0.97. Empirically, on the 3-Enterobacteriaceae air demo this makes the dominant multi-map mode *exactly three* genomes — the real *E. coli* + two *Shigella* core — with under 0.1 % four-or-more-way over-attribution. A contig carrying organism-**specific** sequence aligns more fully to one genome and stays single (the demo's *E. coli* `marA`/`emrE`/`kdpE`/`ugd` did not leak to *Shigella*; *Pseudomonas* `MexA`/`ParR` shared nothing across the family boundary). Each hit records its best-first organism list in `organism_matches`, and the report groups by it so every sibling shows its true repertoire. (`_COMAP_MARGIN` is a single global heuristic — calibrate against a labelled sibling panel before clinical use.)

When no genome map is available (the legacy read-based path, or tests), attribution falls back to underscore-normalised substring matching of the sequence header against organism names.
