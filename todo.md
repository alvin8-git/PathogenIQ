...
| 2 | ~~**Spike-in absolute quantification**~~ — **DONE 2026-06-22**: `quantify.py` (`absolute_copies` = reads_target/reads_spike × copies_spike; `quantify_entries` anchors on a `--spike-taxon`/`--spike-copies` control, drops it from findings, sets per-organism `absolute_copies`; `--sample-volume` → copies/volume). Surfaced in JSON (`absolute_copies`, `copies_per_volume`, `spike_in` block) + TSV. **Renderer display DONE 2026-07-02**: PDF/HTML findings tables show an "Abs. copies" column, shown only when a spike anchored quantification. | High | Clinical load + air surveillance | Med |
| 3 | ~~**Optional assembly + MAG stage**~~ — **DONE 2026-06-22**: `assembly.py` (`run_assembly_stage`: MEGAHIT → minimap2+jgi depth → MetaBAT2 → CheckM ≥50%/≤10% → GTDB-Tk), CLI-gated `--assemble`, non-blocking (skips per-tool if absent; keeps bins when CheckM missing), `MAG` dataclass → JSON `mags` block. Tools added to environment.yml. **PDF/HTML MAG display DONE 2026-07-02** (MAG table/card: bin, GTDB taxonomy, completeness, contamination, contigs, size, open-world grade). **Deferred:** Prokka gene annotation + MAG-level ARG screen. | High | Air surveillance (novel orgs) | High |
